...
class Customer_manage_model extends Base_Model
{
	...
	// public function get_user_all_customer_list($user_id)
	// {
	...
	// 		->result_array();
	// }
	public function get_user_customer_list($user_id, $where_data, $order_by)
	{
		$this->db->select('C.*')->from($this->customer_manage_table . ' as C');
		return $this->db->where($where_data)
			->order_by($order_by)
	...
	public function excel_field_tf_db($data, $user_id)
	{
		...
		foreach ($data as $key => $value) {
			// print_r($data);exit;
			// print_r($customer_no);exit;
			// 有重複編號就給false
			if (in_array($value['A'], $customer_no)) {
				return FALSE;
			}
			// print_r($value['A']);exit;
	...
	//new
	public function get_all_customer_manage_level($user_id)
	{
		$res= $this->db->select('level')
		// $res= $this->db->select('*')
			->from($this->customer_manage_table)
	...
	public function get_all_customer_manage_relation($user_id)
	{
		$res= $this->db->select('level')
		// $res= $this->db->select('*')
			->from($this->customer_manage_table)
	...
	// add 0922
	public function is_customers_num_less_150($user_id)
	{
		if (
			150 <= $this->db->select('id')
			->from('customer_mgr')
			->where(array('is_delete' => 0, 'user_id' => $user_id))
			->get()
			->num_rows()
	...
	// add 0922
	public function is_customers_num_less_600($user_id)
	{
		if (
			600 <= $this->db->select('id')
			->from('customer_mgr')
			->where(array('is_delete' => 0, 'user_id' => $user_id))
			->get()
			->num_rows()
	...
	public function compose_condition_selected($user_id, $data)
	{
		...
		$status 	= array(1 => '未開發',2 => '告知', 3 => '約訪', 4 => '拜訪', 5 => '建議', 6 => '成交');
	...
	// add 0922
	public function get_connection_hot($user_id, $customer_id)
	{
		...
		$res = $this->db->select('*')
			...
			// return $res;
			// print_r($res);exit;
		if(isset($res))$time = strtotime($res['end_date']);
		...
		} else if ($time >= strtotime(Date('Y-m-d H:i:s', strtotime('-3 month')))){
			$data['connection'] = 'red';
		}else if ($time >= strtotime(Date('Y-m-d H:i:s', strtotime('-6 month')))) {
			$data['connection']= 'orange';
		}
		// else if ($time >= strtotime(Date('Y-m-d H:i:s', strtotime('-12 month')))) {
		// 	$data['connection'] = 'yellow';
		// }
		else{
			// $data['date_12']=Date('Y-m-d H:i:s', strtotime('-12 month'));
			$data['end_date']=$res['end_date'];
			$data['connection']= 'yellow';
		}
	...
	public function get_connection_hot_test($user_id, $customer_id)
	{
