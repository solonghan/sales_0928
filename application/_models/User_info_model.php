...
/* Return TRUE or FALSE block - Start */
	...
	// add 0921
	public function can_view_part($user_id)
	{
		$row = $this->db->select('id')
						->from($this->user_table)
						->where(array('id' => $user_id, 'privilege_end_date >=' => date( "Y-m-d H:i:s" )))
		...
		$where=" user_id = $user_id AND (upgrad_plans ='part_1' OR upgrad_plans ='part_12')";
		$row_upgrad = $this->db->select('*')
						->from($this->upgrad_table)
						->where($where)
						// ->where(array('user_id' => $user_id , 'upgrad_plans'=>'part_1'))
						// ->or_where(array('user_id' => $user_id , 'upgrad_plans'=>'part_12'))
						->get()
						->num_rows();
						// ->result_array();
	
		// print_r($result);exit;
		if ($row > 0 && $row_upgrad>0)
		{
			return TRUE;
		}
		else
	...
	// add 0921
	public function can_view_full($user_id)
	{
		$row = $this->db->select('id')
						->from($this->user_table)
						->where(array('id' => $user_id, 'privilege_end_date >=' => date( "Y-m-d H:i:s" )))
		...
		$where=" user_id = $user_id AND (upgrad_plans ='full_1' OR upgrad_plans ='full_12')";
		$row_upgrad = $this->db->select('*')
						->from($this->upgrad_table)
						->where($where)
						// ->where(array('user_id' => $user_id , 'upgrad_plans'=>'part_1'))
...
/* Get block - Start */
	...
	///add 0921
	public function get_part_list($user_id, $keyword = '', $order = 'DESC',$limit='')
	{
		$res = $this->db->select('U.id, U.name, U.avatar')
						->from($this->master_minor_relation_table . ' as M')
						->join('user as U', 'M.minor_id = U.id', 'left')
	...
	// add 0926
	public function get_user_schedule_item($field){

		$res = $this->db->select($field)
						->from($this->schedule_item_table . ' as S')
						->where(array('S.id <=' =>6 ))
						// ->limit($limit)
						->get()
						->result_array();
		...
		}elseif($field=='score_fix'){
			for($i=0;$i<6;$i++){
				$data[]=$res[$i]['score_fix'];
			}
		}
...
/* Set or Add block - Start */
	...
	{
		return $this->db->where('id', $user_id)->update($this->user_table, array('name' => $name));
	}
