...
class Jwt_model extends CI_Model {
	...
	*/
	...
	public function login_token($id)
	{
		...
		$tokenData['create_time'] 	= Date('Y-m-d H:i:s');
		$tokenData['expired_time'] 	= Date('Y-m-d H:i:s',strtotime('+3 months'));
	...
	public function excel_import_token($id, $file_name = '')
	{
		$tokenData['user_id'] 		= $id;
		$tokenData['file_name'] 	= $file_name;
		$tokenData['create_time'] 	= Date('Y-m-d H:i:s');
		$tokenData['expired_time'] 	= Date('Y-m-d H:i:s',strtotime('+3 hours'));
