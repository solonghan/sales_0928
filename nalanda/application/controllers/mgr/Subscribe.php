...
class Subscribe extends Base_Controller
{
    private $th_title = ["#", "用戶名稱", "ID", "動作"]; //, "置頂"
    ...
    public function __construct()
    {
        ...
        $this->action = base_url() . "mgr/subscribe/";
        $this->param = [
            //																								md 		sm
    ...
    {
        $this->data['title'] = '追蹤列表';

        $this->data['custom_data_url'] = base_url() . 'mgr/subscribe/data/' . $user_id;
    ...
    public function data($user_id)
    {
        ...
        $data = $this->Subscribe_model->get_subscribe_list_b($syntax, $order_by, $page, $this->page_count);
        $html = "";
        foreach ($data['list'] as $item) {
            $html .= $this->load->view("mgr/items/subscribe_item", array(
                "item" =>    $item,
            ), TRUE);
        }
    ...
    {
        $this->data['title'] = '粉絲列表';

        $this->data['custom_data_url'] = base_url() . 'mgr/subscribe/fans_data/' . $user_id;
        $this->data['action'] = $this->action;
        $this->data['th_title'] = $this->th_title;
        $this->data['th_width'] = $this->th_width;
        $this->data['can_order_fields'] = $this->can_order_fields;
    ...
    public function fans_data($user_id)
    {
        $page        = ($this->input->post("page")) ? $this->input->post("page") : 1;
        $search      = ($this->input->post("search")) ? $this->input->post("search") : "";
        $order       = ($this->input->post("order")) ? $this->input->post("order") : 0;
        ...
        $data = $this->Subscribe_model->get_fans_list_b($syntax, $order_by, $page, $this->page_count);
        $html = "";
        foreach ($data['list'] as $item) {
            $html .= $this->load->view("mgr/items/fans_item", array(
                "item" =>    $item,
            ), TRUE);
        }
        if ($search != "") $html = preg_replace('/' . $search . '/i', '<mark data-markjs="true">' . $search . '</mark>', $html);
